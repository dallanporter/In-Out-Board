/******
 *
 * A Digital In & Out Board
 * Dallan Porter
 * dallan@porterdigital.com
 * 
 ******/

// You can listen on secure or non-secure port
var use_secure = false; // Set true if you want to listen on SSL
var non_secure_port = 8080;
var secure_port = 443;
var server;

var db_hostname = "yourhostname";
var db_username = "db_username";
var db_password = "db_password";
var db_database = "inoutboard";
...
var Recaptcha, recaptcha_private_key, recaptcha_public_key;
...
if( use_recaptcha )
{
	Recaptcha = require("recaptcha").Recaptcha; // https://www.npmjs.com/package/recaptcha (for signup)
...
if( use_secure )
{
	privateKey = fs.readFileSync("/path/to/your/private/ssl.key").toString();
	certificate = fs.readFileSync("/path/to/your/chained/cert_chain.crt").toString();
}
...
app.get( "/signup", function( req, res ) {
	// NOTE - this part is not working yet. You must create groups & users manually in the database.
	if( use_recaptcha )
	{
		var recaptcha = new Recaptcha( recaptcha_public_key, recaptcha_private_key );
		var mod_output = recaptcha.toHTML();
...
if( use_secure )
{
	server = https.createServer( options, app ); // use express
}
else
{
	server = http.createServer( app ); // use express
}

var io = require("socket.io").listen( server ); // bind Socket.io to the webserver
if( use_secure )
{
	server.listen( secure_port ); // listen for HTTPS
}
else
{
	server.listen( non_secure_port ); // also listen for non-secure HTTP
}
